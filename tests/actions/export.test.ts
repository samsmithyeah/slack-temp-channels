...
describe("registerExportAction", () => {
    ...
    it("acks and opens the export modal with channel info", async () => {
      ...
      await app.handlers["action:/^home_export_/"]({
        ...
        body: {
          ...
          user: { id: "U_USER" },
          actions: [
            { type: "button", value: "C_CHAN:my-channel", action_id: "home_export_C_CHAN" },
          ],
        },
        client,
    ...
    it("handles channel names with colons in value", async () => {
      ...
      await app.handlers["action:/^home_export_/"]({
        ...
        body: {
          ...
          user: { id: "U_USER" },
          actions: [
            { type: "button", value: "C_CHAN:name:with:colons", action_id: "home_export_C_CHAN" },
          ],
        },
        client,
        logger: createMockLogger(),
  ...
  describe("home_export action opens correct modal", () => {
    it("opens export modal for text-only export", async () => {
      const ack = vi.fn();
      const client = createMockClient();
    ...
    it("opens export-with-files modal for file export", async () => {
      const ack = vi.fn();
      const client = createMockClient();
      ...
      await app.handlers["action:/^home_export_/"]({
        ...
        body: {
          ...
          user: { id: "U_USER" },
          actions: [
            { type: "button", value: "C_CHAN:my-channel", action_id: "home_export_files_C_CHAN" },
          ],
        },
        client,
        logger: createMockLogger(),
  ...
  describe("export_with_files_submit view", () => {
    it("registers the view handler", () => {
      expect(app.handlers["view:export_with_files_submit"]).toBeDefined();
    });

    it("uploads a zip file to the DM channel", async () => {
      const ack = vi.fn();
      const client = createMockClient();
      (client as unknown as { token: string }).token = "xoxb-test-token";
      setupExportClient(client, "U_REQUESTER");
      client.conversations.history.mockResolvedValue({
        messages: [{ user: "U1", text: "Hello", ts: "1700000000.000000" }],
        response_metadata: {},
      ...
      await app.handlers["view:export_with_files_submit"]({
        ack,
        view: {
          private_metadata: "C_CHAN:test-channel",
          state: {
    ...
    it("sends a 'preparing export' message before downloading", async () => {
      ...
      expect(client.chat.postMessage).toHaveBeenCalledWith(
        expect.objectContaining({
          channel: "D_DM",
          text: expect.stringContaining("Preparing export"),
        }),
      );
    });
